...
{
	if (NULL == m_pRobot->m_pcEcatMaster[0])
		return FALSE;
	...
	try
	{
		...
		{
		btMasterState = m_pRobot->m_pcEcatMaster[0]->GetMasterState();
		btSlaveState = m_pRobot->m_pcEcatMaster[0]->GetSlaveState();
		btDomainState = m_pRobot->m_pcEcatMaster[0]->GetDomainState();
		usSlaveNo = (UINT16)m_pRobot->m_pcEcatMaster[0]->GetSlaveCnt();
		}
