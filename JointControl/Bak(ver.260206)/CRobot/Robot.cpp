...
CRobot::Init(BOOL abSim)
{
	...
	if (FALSE == abSim)
	{
		if (m_cIMU.GetImuEnabled()) {
			if (FALSE == m_cIMU.Connect()) {
				DBG_LOG_ERROR("(%s) Cannot IMU Connection", "CRobot");
				return FALSE;
			}
		}
		
		// Gamepad 초기화 (Gamepad 클래스 내부에서 모든 처리 담당)
		if (m_pcGamepad != NULL)
		{
			m_pcGamepad->Init();
		}
...
CRobot::DeInit()
{
	...
	for (int i=0; i<NUM_OF_MASTER; i++)
		if (m_pcEcatMaster[i] != NULL)
			m_pcEcatMaster[i]->DeInit();
...
CRobot::InitRTTasks()
{
	...
	for (int nCnt = 0; nCnt < nTotalRtTasks; nCnt++)
	{
		...
		if (vTaskConf[nCnt].strName == "EtherCAT Control Task")
			set_cpu_affinity(&taskTemp, 6);
		if (vTaskConf[nCnt].strName == "EtherCAT Control Task2")
			set_cpu_affinity(&taskTemp, 7);
...
CRobot::UpdateExtInterfaceData()
{
	if (m_pcExtInterface)
	{
		if (m_pcEcatMaster[0])
		{
			m_pcExtInterface->UpdateEcatMetadata((UINT16)m_pcEcatMaster[0]->GetSlaveCnt(), m_pcEcatMaster[0]->GetMasterState(), m_pcEcatMaster[0]->GetSlaveState(), m_pcEcatMaster[0]->GetDomainState());
		}
	}
}
...
{
	if (m_pcEcatMaster[0] && m_pcEcatMaster[1])
		return TRUE;
...
CRobot::GetTotalEcatSlaves()
{
	if (TRUE == IsEcatEnabled())
		return (m_pcEcatMaster[0]->GetSlaveCnt()+m_pcEcatMaster[1]->GetSlaveCnt());
...
BOOL CRobot::AddHand(CHand* apHand)
{
	if (0 > GetTotalHands())
	{
		for (int nCnt = 0; nCnt < GetTotalHands(); nCnt++)
		{
			if (apHand->GetName() == m_vHand[nCnt]->GetName())
			{
				return FALSE;
			}
		}
